    ...
    if default is not None and default >= 0:
        ...
        if peak is not None:
            if peak >= INPUT_SILENT_RMS:
                ...
                log.info(
                    "Using default microphone [%d]: %s (probe rms=%.5f, room is quiet)",
                    default,
                    default_name,
                    peak,
                )
            return default
        log.warning(
            "Default mic [%d] %s is unavailable/unopenable; scanning other inputs...",
            default,
            default_name,
        )
    ...
    try:
        with sd.InputStream(
            ...
            while True:
                ...
                print(f"RMS={level:.5f}  Noise={noise_floor:.5f}  Threshold={threshold:.5f}")
                if level >= threshold:
                    print(f"CLAP CANDIDATE: {level:.5f}")
